...
LINUX_ASM_SRCS := src/asm/linux/scan_core.asm src/asm/linux/network.asm src/asm/linux/data.asm src/asm/linux/thread.asm
WIN_ASM_SRCS   := src/asm/windows/scan_core.asm src/asm/windows/network.asm src/asm/windows/data.asm src/asm/windows/thread.asm
...
$(LINUX_OBJ_DIR)/%.o: src/asm/linux/%.asm include/netrox-asc_abh.h
	@mkdir -p $(LINUX_OBJ_DIR)
	$(NASM) -f elf64 -D LINUX -I src/asm/common/ $< -o $@

$(WIN_OBJ_DIR)/%.obj: src/asm/windows/%.asm include/netrox-asc_abh.h
	@mkdir -p $(WIN_OBJ_DIR)
	$(NASM) -f win64 -D WINDOWS -I src/asm/common/ $< -o $@

$(LINUX_OBJ_DIR)/cpp/%.o: src/cpp/%.cpp include/netrox-asc_abh.h
	@mkdir -p $(LINUX_OBJ_DIR)/cpp
	$(CXX) $(CXXFLAGS) -c $< -o $@

$(WIN_OBJ_DIR)/cpp/%.obj: src/cpp/%.cpp include/netrox-asc_abh.h
